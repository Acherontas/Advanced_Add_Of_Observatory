...
using namespace std;
   ...
   long long int hlp_cdi=0;
   long long int hpl_icd=0;
   //

    sum_cdi *smcd=new sum_cdi();

    simpl_obser *simpobser=new simpl_obser();
...
int using_protous(long long int main_number,long long int first_cdi,long long int sum_of_nines,long long int last_cdi)
{
   ...
   cout<<"      and " << main_number << " - " << tbs_rst << " ::== " << frs_tbs <<" with cdi ";
   hlp_cdi=smcd->cdis(abs(frs_tbs),4);
   cout<<hlp_cdi <<"\n";
   rfs_sbt=sum_of_nines-frs_tbs;
   cout<<"      --> " << sum_of_nines << " - " << frs_tbs << " ::== " << rfs_sbt <<"\n";
   rst_sb=rfs_sbt-tbs_rst;
   cout<<"      --> " << rfs_sbt << " - " << tbs_rst << " ::== " << rst_sb <<" with cdi ";
   hpl_icd=smcd->cdis(abs(rst_sb),4);
   cout<<hpl_icd <<"\n";
   confi=stb_nn-rst_sb;
   ...
   cout<<"\n";
   cout<<"\n";
   simpobser->expose_ari(hlp_cdi,hpl_icd);
   cout<<"\n";
   simpobser->expose_dex(hlp_cdi,hpl_icd);
   cout<<"\n";
   return 0;
}
